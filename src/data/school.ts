// DATA SUMBER: hasil riset lapangan yang dipublikasikan di jurnal akademik
// (Journal of Social Research Vol.3 No.4 2024; Tarbiyatuna Unimma; AL-ULUM IIM Surakarta)
// serta pemberitaan media. Field bertanda [DATA RESMI] masih menunggu konfirmasi sekolah.

export const school = {
  name: "STTD Al-Busyro",
  fullName: "Sekolah Tahfizhul Qur'an Tingkat Dasar Al-Busyro",
  shortName: "Al-Busyro",
  logo: '/logo_.png',
  tagline: "Membentuk Generasi Qur'ani dengan Adab dan Ilmu",
  description:
    "STTD AL-BUSYRO adalah Sekolah Tahfizhul Qur’an Tingkat Dasar Mutqin Hafalannya, Fasih Bacaannya, Baik Adabnya, Diakui Legalitasnya.",
  longDescription:
    "STTD Al-Busyro Adalah sekolah dasar (SD) yang memiliki program tahfizh intensif (mengutamakan baiknya bacaan dan lancarnya hafalan) yang dipadukan dengan kurikulum kementrian agama (Madrasah). kami berupaya memberikan lingkungan Islami yang mendukung tumbuh kembang anak sesuai dengan fitrah dan potensi yang telah Allah berikan. Sehingga anak memiliki bekal dasar untuk menyambut masa depan cemerlang yang penuh dengan tantangan.",
    story: "Sejarah SIngkat STTD Al-Busyro Sekolah Tahfizhul Qur’an Tingkat Dasar, atau lebih dikenal dengan STTD Al-Busyro, merupakan lembaga pendidikan Islam yang didirikan sebagai salah satu wujud cita-cita dan harapan Ust. Abu Hurri Al-Qosimi dalam rangka membumikan Al-Qur'an. Sebagai pendiri STTD Al-Busyro sekaligus penggagas Metode Al-Qosimi, beliau mengembangkan pendidikan yang menempatkan Al-Qur'an sebagai prioritas utama dalam proses pembelajaran. STTD Al-Busyro merupakan sekolah setingkat SD yang memberikan perhatian khusus terhadap pendidikan Al-Qur'an. Kegiatan Tahfizh Al-Qur'an menjadi prioritas utama, dengan perhatian tidak hanya pada jumlah hafalan, tetapi juga pada kelancaran dan kemantapan hafalan (mutqin). Di samping pendidikan Al-Qur'an, STTD Al-Busyro tetap memberikan pembelajaran umum sebagai bekal bagi peserta didik dalam membangun dasar-dasar ilmu yang diperlukan untuk kehidupan mereka di masa mendatang. Dengan demikian, pendidikan di STTD Al-Busyro diarahkan untuk membentuk generasi yang tidak hanya memiliki hafalan Al-Qur'an, tetapi juga memiliki bacaan yang baik, adab yang mulia, wawasan yang memadai, dan kesiapan untuk bersaing di masa depan.",
  city: "Surakarta",
  address: "Kuncen, RT 05 / RW 03, Desa Waru, Kec. Baki, Kabupaten Sukoharjo, Jawa Tengah",
  phone: "085 726216717",
  phoneContact: "Ust Mihwar",
  whatsapp: "6285726216717",
  email: "sekolahtahfizhalbusyro@gmail.com",
  social: {
    instagram: "@sttdalbusyro",
    facebook: "sttdalbusyro",
    youtube: "[DATA RESMI BELUM TERSEDIA]",
    whatsapp: "[DATA RESMI BELUM TERSEDIA]",
  },
  motto: "Bersama Mencetak Generasi Qur’ani",
  vision:
    "Mencetak generasi Qur’ani yang berakhlaq Islami dan bermanhaj salafush shalih.",
  mission: [
    "Menanamkan aqidah shahihah kepada anak didik.",
    "Membimbing anak menghafal Al-Quran dan Al-Hadits.",
    "Membekali anak dengan ilmu-ilmu Al-Quran.",
    "Membekali anak dengan ilmu-ilmu syar’i yang sesuai dengan perkembangannya.",
    "Memberikan keteladanan yang sesuai dengan Al-Quran dan As-Sunnah.",
  ],
  method: {
    name: "Metode Al-Qosimi",
    founder: "Ust. Abu Hurri Al-Qosimi Al-Hafidz",
    konsep: "Konsep S3 Perawatan",
    description:
      "Metode tahfizh yang diciptakan langsung oleh pendiri STTD Al-Busyro. Santri membaca ayat yang akan dihafal secara berulang hingga terbentuk gambaran kuat di ingatan, kemudian menghafalnya — terbukti cocok untuk anak-anak hingga dewasa dengan.",
    variants: [
      {
        name: "Sebelum",
        detail: "Membaca berulang 40x (atau 20x bagi usia emas) sebelum mengafal",
      },
      {
        name: "Sedang",
        detail: "Proses membaca berulang sampai lancar",
      },
      {
        name: "Setelah",
        detail: "Mengulang hafalan baru yang belum diseor sekali duduk",
      },
      {
        name: "Perawatan",
        detail: "Muroja'ah hafalan lama yang tersruktur untuk menjaga hafalan lama",
      },
    ],
  },
  values: [
    {
      title: "Al-Qur'an",
      description:
        "Membina bacaan yang fasih, hafalan yang mutqin, dan kecintaan mendalam terhadap Al-Qur'an.",
      icon: "BookOpen" as const,
    },
    {
      title: "Adab",
      description:
        "Menghormati guru, orang tua, dan majelis ilmu. Disiplin dalam beribadah dan bergaul.",
      icon: "Heart" as const,
    },
    {
      title: "Ilmu",
      description:
        "Menuntut ilmu dengan sungguh-sungguh dan mengamalkannya dalam kehidupan sehari-hari.",
      icon: "GraduationCap" as const,
    },
    {
      title: "Karakter",
      description:
        "Membentuk pribadi yang mandiri, bertanggung jawab, dan memiliki kepedulian terhadap sesama.",
      icon: "Users" as const,
    },
    {
      title: "Kemandirian",
      description:
        "Menumbuhkan kemampuan untuk berdiri sendiri dan menghadapi tantangan dengan percaya diri.",
      icon: "Shield" as const,
    },
    {
      title: "Kepedulian",
      description:
        "Menumbuhkan kepekaan terhadap sesama manusia dan menjaga kelestarian lingkungan.",
      icon: "HandHeart" as const,
    },
  ],
  educationValues: {
    relationWithAllah: {
      title: "Hubungan dengan Allah",
      items: ["Ibadah", "Doa", "Al-Qur'an", "Ketakwaan", "Akhlak Islami"],
    },
    relationWithHumans: {
      title: "Hubungan dengan Manusia",
      items: [
        "Saling membantu",
        "Menghormati",
        "Sopan santun",
        "Kepedulian",
        "Kerja sama",
      ],
    },
    relationWithNature: {
      title: "Hubungan dengan Alam",
      items: [
        "Menjaga kebersihan",
        "Menjaga lingkungan",
        "Bertanggung jawab terhadap alam",
      ],
    },
  },
  whyChooseUs: {
    headline: "Kenapa STTD Al-Busyro Hadir?",
    description:
      "Pendidikan bukan hanya tentang nilai akademik. Setiap orang tua tentu menginginkan anak yang cerdas. Namun, di atas segala pencapaian, ada kebahagiaan yang tak ternilai: melihat anak tumbuh mencintai Al-Qur'an, berakhlak mulia, dan menjadi penyejuk hati keluarga.",
    threeMainPoints: [
      {
        title: "Al-Qur'an Berkualitas",
        subtitle: "Bukan sekadar mengejar banyak hafalan",
        items: [
          "Bacaan benar (Fasih)",
          "Hafalan Lancar ( Target 10 - 15 Juz Mutqin)",
          "Dibawah Bimbingan Ustadz Abu Hurri Al- Qosimi Al-Hafizh",
        ],
      },
      {
        title: "Adab",
        subtitle: "Budaya hormat dan disiplin",
        items: [
          "Menghormati Guru dan Orang Tua",
          "Menghormati Majelis Ilmu",
          "Disiplin Beribadah",
        ],
      },
      {
        title: "Akademik",
        subtitle: "Ijazah resmi dan siap melanjutkan",
        items: [
          "Ijazah Resmi Kemenag (MI)",
          "Memiliki NISN",
          "Siap Melanjutkan dengan Kurikulum Terpadu",
        ],
      },
    ],
  },
  programs: {
    sekolah: {
      title: "Program Unggulan",
      description:
        "Wajib bagi seluruh siswa: pelajaran umum menunjang pengetahuan dan persiapan ujian kenegaraan, diseimbangkan dengan pendidikan agama agar siswa tidak hanya fokus pada hafalan.",
    },
    asrama: {
      title: "Program Asrama",
      description:
        "Pembinaan intensif di bawah pengasuh untuk menambah kapasitas hafalan sekaligus melatih kemandirian santri dalam menjalankan kewajiban harian.",
    },
    targetNonAsrama: "10 - 15 Juz Mutqin",
    targetAsrama: "Mengutamakan Bacaan Fasih",
  },
  activities: [
    "Mabit",
    "PBB",
    "Life Skill",
    "Outbond",
    "Renang",
    "Berkuda",
    "Public Speaking",
    "Memanah",
    "Tartil dan Tilawah",
    "Beladiri",
    "Jurnalistik",
    "Sains clup"
  ],
  cta: {
    headline: "Setiap Langkah Kecil Hari Ini Akan Menentukan Masa Depan Mereka",
    description:
      "Mari bergabung bersama STTD Al-Busyro dan dampingi putra-putri Anda bertumbuh menjadi generasi yang mencintai Al-Qur'an, beradab, dan berprestasi.",
    primaryButton: "Informasi PPDB",
    secondaryButton: "Hubungi Kami",
  },
  get copyright() {
    return `© ${new Date().getFullYear()} STTD Al-Busyro. All rights reserved.`;
  },
};
