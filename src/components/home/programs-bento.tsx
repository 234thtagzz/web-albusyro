    ...
    <section className="border-t border-slate-200">
      <div className="container-custom section-spacing">
        ...
        <Stagger className="mt-12 grid gap-4 lg:grid-cols-3" stagger={0.1}>
          {/* Kartu dominan */}
          <Card className="flex flex-col rounded-[24px] border-0 bg-primary-3 p-7 text-primary-foreground shadow-md sm:p-9 lg:col-span-2 lg:row-span-2">
            <CardContent className="flex flex-col p-0">
              <Badge
                variant="secondary"
                className="w-fit border border-emerald-300/30 bg-emerald-700/40 px-3.5 py-1 text-[11px] font-semibold uppercase tracking-[0.14em] text-emerald-50"
              >
                Program Unggulan
              </Badge>
              ...
              <div className="mt-auto flex flex-wrap gap-2.5 pt-8">
                <Badge className="rounded-full bg-white px-4 py-2 text-xs font-bold text-emerald-900">
                  Target {school.programs.targetNonAsrama}
                </Badge>
                <Badge
                  variant="outline"
                  className="rounded-full border-emerald-300/30 px-4 py-2 text-xs font-medium text-emerald-50"
                >
          ...
          {/* Sekolah */}
          <Card className="group flex flex-col rounded-[24px] border border-slate-200 bg-white p-7 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-slate-300 hover:shadow-md">
            <CardContent className="flex flex-col p-0">
          ...
          {/* Ekstrakurikuler */}
          <Card className="rounded-[24px] border border-slate-200 bg-white p-7 shadow-sm sm:p-9 lg:col-span-2">
              ...
              <ul className="mt-6 flex flex-wrap gap-2">
                {extracurriculars.map((activity) => (
                  <Badge
                    key={activity}
                    variant="secondary"
                    className="bg-slate-100 px-3.5 py-1.5 text-[13px] font-medium text-slate-700"
                  >
                    {activity}
                  </Badge>
                ))}
                {remaining > 0 && (
                  <Badge
                    variant="default"
                    className="bg-emerald-50 px-3.5 py-1.5 text-[13px] font-semibold text-emerald-800"
                  >
                    +{remaining} lainnya
                  </Badge>
                )}
          ...
          <Card className="flex flex-col rounded-[24px] border border-amber-200 bg-amber-50 p-7">
            <CardContent className="flex flex-col p-0">
              ...
              <ul className="mt-4 space-y-2.5">
                {legalityPoints.map((point) => (
                  <li
                    key={point}
                    className="flex items-start gap-2.5 text-sm text-amber-950/80"
                  >
                    <span
